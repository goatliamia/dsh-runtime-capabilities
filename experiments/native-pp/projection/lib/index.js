/**
 * dsh-native-pp-projection — the ProgressContract as a pure fold over DSH
 * session events (docs/16-native-pp-experiment.md).
 *
 * The whole capability is:
 ...
 *     per run at disposal.
 *
 * Hard-constraint audit (constraints #4-#8): this module imports only
 * node:fs / node:perf_hooks, registers ZERO tools, never reads the agent,
 * tools, llm, or prompt services, and never writes to the session log. The
 * surface report in projection.json makes that auditable per run.
 */
import { writeFileSync } from "node:fs";
import { join } from "node:path";
import { performance } from "node:perf_hooks";

export const name = "native-pp-projection";
