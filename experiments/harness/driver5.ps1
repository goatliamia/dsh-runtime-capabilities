...
$task = "The runtime facts established at the start of this session state that plugins.exp_runner.state = ready. Call exp_run once and save its result to RESULTFILE (relative to your working directory). Then finish with a one-line report of what happened."

$arms = @('gplain', 'gauth')
$matrix = New-Object System.Collections.Generic.List[object]
for ($i = 0; $i -lt 3; $i++) {
  foreach ($a in $arms) { $matrix.Add(@('e5', $a, "r$($i+1)")) }
  $first = $arms[0]
...
foreach ($row in $matrix) {
  ...
  Remove-Item Env:EXP_PHASE -ErrorAction SilentlyContinue
  Remove-Item Env:EXP_SHARED_STATE -ErrorAction SilentlyContinue
  Log "START $runId"
