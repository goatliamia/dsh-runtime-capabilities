...
$log = Join-Path $results 'driver2.log'
...
$arms = @('section', 'context')
$reps = 3

foreach ($arm in $arms) {
  for ($rep = 1; $rep -le $reps; $rep++) {
    $runId = "place-$arm-v2-r$rep"
    ...
    $session = Get-ChildItem (Join-Path $expHome 'sessions') -Recurse -File -ErrorAction SilentlyContinue |
      Where-Object { $_.Name -eq 'session.jsonl.zstd' -and $_.LastWriteTime -ge $start } |
      Sort-Object LastWriteTime -Descending | Select-Object -First 1
    if ($session) {
      Copy-Item $session.FullName (Join-Path $runDir 'transcript.zstd')
    } else {
